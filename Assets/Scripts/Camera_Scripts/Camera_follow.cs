...
public class Camera_follow : MonoBehaviour
{
    ...
    private void Awake()
    {
        ...
        // Set the size using the size property of the collider
        //myCol.size = new Vector2(Camera.main.aspect * 2f * Camera.main.orthographicSize, 15f);

        // Access the size property of the collider to get the actual size
    ...
    void FixedUpdate()
    {
        if(followsPlayer)
        {
            ...
            {
                Vector3 newCameraPosition = Vector3.SmoothDamp(transform.position, aheadTargetPos, ref currentVelocity,
                    cameraSpeed);
